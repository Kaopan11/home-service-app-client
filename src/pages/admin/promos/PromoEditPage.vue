...
import { buildPromoPayload, splitPromoExpiry, type PromoFormState } from '@/data/adminPromos'
import { formatAdminDateTime } from '@/data/adminServices'
import { deletePromotion, getPromotion, updatePromotion } from '@/services/promoApi'
...
const canSubmit = computed(() => Boolean(form.code.trim()) && !submitting.value && Boolean(item.value))
...
onMounted(async () => {
  ...
  try {
    ...
    form.discountType = item.value.discount_type
    form.quota = String(item.value.quota_limit)
    if (item.value.discount_type === 'percent') {
      form.percentValue = String(item.value.discount_value)
    } else {
      form.fixedValue = String(item.value.discount_value)
    }
    const expiry = splitPromoExpiry(item.value.expires_at)
    form.expiryDate = expiry.date
    form.expiryTime = expiry.time
  } catch (err) {
...
async function handleSubmit(): Promise<void> {
  ...
  error.value = ''
  try {
    await updatePromotion(item.value.promotion_id, result.payload)
    await router.push({ name: 'admin-promos' })
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'ไม่สามารถแก้ไข Promotion Code ได้'
  } finally {
...
async function confirmDelete(): Promise<void> {
  ...
  error.value = ''
  try {
    await deletePromotion(item.value.promotion_id)
    await router.push({ name: 'admin-promos' })
  } catch (err) {
    ...
    <template #topbar>
      <div class="heading-wrap">
        ...
        <div class="heading">
          ...
          <h1 class="page-title">{{ item?.code || 'แก้ไข Promotion Code' }}</h1>
        </div>
      </div>
    ...
    <template v-else-if="item">
      ...
      <form class="card" @submit.prevent="handleSubmit">
        ...
        <div class="meta">
          ...
          <div class="meta-row">
            <span class="meta-label">แก้ไขล่าสุด</span>
